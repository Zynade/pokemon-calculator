...
def input_pokemon_id():
    while True:
        pokemon_id = input(
            "Enter your pokemon name or enter a number between 1 to 898: ")
        try:
...
def main():
    ...
    info_on_pokemon1 = json.loads(
        (requests.get(api_url + "/pokemon/" + pokemon_ally_id).text))
    pokemon_ally = dict()
    ...
    info_on_pokemon2 = json.loads(
        (requests.get(api_url + "/pokemon/" + pokemon_enemy_id).text))
